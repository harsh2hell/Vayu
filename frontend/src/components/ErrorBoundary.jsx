          ...
          <div className="max-w-md w-full bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 shadow-2xl text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-2.5 justify-center">
              <button
                ...
                className="px-4 py-2.5 rounded-2xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-semibold text-xs transition-colors shadow-lg shadow-cyan-500/20"
              >
                Clear Cache & Reload
              </button>
              <button
                onClick={() => window.location.href = '/'}
                className="px-4 py-2.5 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-medium text-xs transition-colors border border-slate-700"
              >
                Back to Home
              </button>
              <button
                onClick={() => window.location.reload()}
                className="px-4 py-2.5 rounded-2xl bg-slate-800/60 hover:bg-slate-700/60 text-slate-400 font-medium text-xs transition-colors border border-slate-800"
              >
                Try Again
              </button>
            </div>
            {this.state.error && (
              <details open className="mt-5 text-left text-[11px] text-rose-400 bg-rose-950/30 p-3 rounded-2xl border border-rose-900/40">
